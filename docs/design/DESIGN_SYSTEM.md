...
### 5.16 스크랩·스트릭 — ScrapToggle / StreakBadge / StreakCard (리뷰 반영, 2026-07-16 오너 결정 갱신)
- **ScrapToggle** — 문제를 개인 북마크에 저장/해제하는 토글(별·북마크 아이콘, secondary). 켜짐=`primary`, 꺼짐=`textTertiary`. 문제 풀이 화면(03) 및 스크랩 목록에서 사용.
- **스트릭 표시는 맥락에 따라 두 형태로 나뉜다** — 색 매핑(`streakTone`)은 하나로 공유하고 그릇만 다르다.
  - **StreakCard**(02 홈) — 지속 동기의 핵심 장치라 알약 배지가 아니라 **독립 카드로 승격**한다(2026-07-16 오너 결정). 카드 표면은 중립(BcsCard), 아이콘 타일 색만 `streakTone`을 따른다(끊겨도 카드 전체가 물들지 않는다). "N일째 꾸준히 한입!" 한 줄. ⚠️ "내일도 오시면…" 같은 미래 문구는 손실 프레임 부담 우려로 **쓰지 않는다** — 오늘 성취만 말한다.
