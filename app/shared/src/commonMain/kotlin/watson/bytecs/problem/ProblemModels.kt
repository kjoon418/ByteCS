 ...
 * (추가 학습으로 통일) 나면서 이 패키지는 '피처 슬라이스'가 아니라 **공용 도메인 leaf + HTTP 인프라**
 * 홈으로 남는다. 아래 타입들은 session·scrap·extrastudy 슬라이스가 공유한다(중복 정의 방지).
 */
 ...
 * 연장 — 문제 배포 응답에는 절대 포함되지 않는다). session·scrap·extrastudy 슬라이스가 이 타입을 공유한다.
