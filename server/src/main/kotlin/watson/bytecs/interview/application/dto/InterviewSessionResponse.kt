 ...
 *
 * currentHintCount·currentRevealedHints는 현재 질문의 힌트 개수·이미 공개한 힌트(재진입 복원)다(SessionProblemResponse
 * 관례 미러) — 힌트 열람은 채점·준비도·쿼터와 무관한 무낙인 정보라 no-leak 제약과 독립이다. 현재 질문이 없으면(완료) 둘 다 null.
 */
data class InterviewSessionResponse(
    ...
    val currentRevealedHints: List<String>?,
)
